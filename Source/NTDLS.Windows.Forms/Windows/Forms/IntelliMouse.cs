...
//			this.Visible =false;
		...
		protected void InitializeComponent()
		{
			...
			this.tmrFeedback = new NTDLS.Timers.WeakTimer(this.components);
			this.picImage = new System.Windows.Forms.PictureBox();
            this.regionHandler1 = new NTDLS.Windows.Forms.RegionHandler(this.components);
			// 
			// tmrFeedback
			// 
		...
		protected void SetCursor(int x, int y)
		{
			...
			if (dY > 16)
			{
                this.Cursor = new Cursor(assembly.GetManifestResourceStream("NTDLS.MoveDown.cur"));
				CurrentDelta.Y -= 16;

			}
			else if (dY < -16)
			{
                this.Cursor = new Cursor(assembly.GetManifestResourceStream("NTDLS.MoveUp.cur"));
				CurrentDelta.Y += 16;

			}
			else
			{
                Stream strm = assembly.GetManifestResourceStream("NTDLS.MoveUpDown.cur");
