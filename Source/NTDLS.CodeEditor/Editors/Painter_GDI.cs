...
namespace NTDLS.Windows.Forms.CodeEditor.Painter
{
    /// <summary>
    ...
    public class Painter_GDI : IPainter
    {
        ...
        public Point PointToCurrentScreen(Control ctrl, Point location)
        {
            var screenBounds = Screen.FromControl(ctrl).Bounds;
            var globalCoordinates = ctrl.PointToScreen(location);
        ...
        private void RenderAll2(Graphics g)
        {
            ...
            if (Control.AutoListStartPos != null)
            {
                ...
                if (Control.AutoListVisible)
                {
                    ...
                    {
                        alP = PointToCurrentScreen(Control, alP);

                        Screen screen = Screen.FromControl(Control);

                        alP.X += screen.Bounds.X;
                        alP.Y += screen.Bounds.Y;

                        alP.Y += Control.View.RowHeight + 2;
                        alP.X += -20;

                        if ((alP.Y - screen.WorkingArea.Y) + Control.AutoList.Height > screen.WorkingArea.Height)
                        {
                            alP.Y -= ((alP.Y - screen.WorkingArea.Y) + Control.AutoList.Height) - screen.WorkingArea.Height;
                        }

                        if ((alP.X - screen.WorkingArea.X) + Control.AutoList.Width > screen.WorkingArea.Width)
                        {
                            alP.X -= ((alP.X - screen.WorkingArea.X) + Control.AutoList.Width) - screen.WorkingArea.Width;
                        }
