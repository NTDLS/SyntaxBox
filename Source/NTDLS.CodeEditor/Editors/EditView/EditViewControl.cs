...
using NTDLS.Windows.Forms.CodeEditor.Painter;
using NTDLS.Windows.Forms.CodeEditor.TextDraw;
using ScrollEventArgs = NTDLS.Windows.Forms.IntelliMouse.ScrollEventArgs;
using ScrollEventHandler = NTDLS.Windows.Forms.IntelliMouse.ScrollEventHandler;
using NTDLS.Win32;
using System.IO;
...
namespace NTDLS.Windows.Forms.CodeEditor
{
	...
	public sealed class EditViewControl : SplitViewChildWidget
	{
        ...
        /// <summary>
        /// Fired when the text is not found.
        /// </summary>
        public event FindReplaceTextNotFoundHandler TextNotFound = null;
		...
		private void CreateFindForm()
		{
			if (!this._CodeEditor.DisableFindForm && this._FindReplaceDialog == null)
			{
                ...
                FindReplaceDialog.TextNotFound += new FindReplaceTextNotFoundHandler(_FindReplaceDialog_TextNotFound);
			}
		}
        ...
        void _FindReplaceDialog_TextNotFound(object sender, string text)
        {
            if (TextNotFound != null)
                TextNotFound(null, text);
        }
		...
		public void ScrollScreen(int Amount)
		{
			ScrollScreen(Amount, 2);
		}

        public void ScrollScreen(int Amount, int speed)
        {
            //try
            //{
			...
			else
			{
				if (x < View.TotalMarginWidth)
				{
					...
					else
					{
                        ...
                        Stream strm = assembly.GetManifestResourceStream("NTDLS.FlippedCursor.cur");
...
//				go.Owner=(Form)this.TopLevelControl;
            ...
            for (int i = this.Caret.Position.Y; i < this.Document.Count; i++)
            {
                ...
                else
                {
                    ...
                    do
                    {
                        ...
                        Col = t.IndexOf(pattern, StartCol);
        ...
        public void ShowFind(string defaultText)
		{
			if (FindReplaceDialog != null)
			{
                ...
                FindReplaceDialog.ShowFind(defaultText);
			}
		}
        ...
        public void ShowReplace(string defaultText)
        {
            if (FindReplaceDialog != null)
            {
                ...
                FindReplaceDialog.ShowReplace(defaultText);
            }
        }
        ...
        {
            get { return this.AutoList.Items; }
        }
		...
		protected override void OnDragDrop(DragEventArgs drgevent)
		{
			if (!this.ReadOnly)
			{
				if (Document != null)
				{
                    ...
                    object droppedObject = drgevent.Data.GetData(typeof(string));

                    if (droppedObject != null)
                    {
                        string s = droppedObject.ToString();
